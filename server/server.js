...
app.get("/currencies/:id", (req, res) => {
  ...
  db.getCurrencyByID(id)
    ...
    .catch((err) => {
      res
        .status(500)
        .send({ error: "Failed to fetch currency", message: err.message });
    });
});
...
app.post("/rules", (req, res) => {
  ...
  db.addRule(body)
    ...
    .catch((err) => {
      res
        .status(400)
        .send({ error: "Failed to add rule", message: err.message });
    });
});
...
app.get("/rules/:id", (req, res) => {
  ...
  db.getRuleByID(id)
    ...
    .catch((err) => {
      res
        .status(500)
        .send({ error: "Failed to fetch rule", message: err.message });
    });
});
...
app.post("/shows", (req, res) => {
  ...
  db.addShow(body)
    ...
    .catch((err) => {
      res
        .status(400)
        .send({ error: "Failed to add show", message: err.message });
    });
});
  ...
  db.getMaintenanceRequestById(id)
    .then((data) => {
      if (!data) {
        return res
          .status(404)
          .send({ error: `Maintenance request not found: ${id}` });
      }
      res.send(data);
    })
    .catch((err) => {
