  ...
  function renderPage(page) {
    ...
    pageData.forEach((req) => {
      ...
      row.innerHTML = `
        ...
        <td>${req.installationCustomerId?.name || "غير متوفر"}</td>
        <td>${req.installationDeviceType || "غير متوفر"}</td>
        <td>
          <span class="badge bg-${
            req.installationPriority === "عالية"
              ? "danger"
              : req.installationPriority === "متوسطة"
              ? "warning"
              : "info"
          }">
            ${req.installationPriority || "غير محدد"}
          </span>
        </td>
        <td>
          <span class="badge bg-${
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  window.printTable = function (tableID) {
    ...
    printWindow.document.write(`
      <html>
        ...
        <body>
          <h2 class="text-center mb-4">طلبات التركيب</h2>
          ${table.outerHTML}
    ...
    exportButtons.forEach((btn) => {
      btn.addEventListener("click", (e) => {
        ...
        if (exportType === "excel") {
          ...
          window.printTable("installationRequestsTable");
        }
      });
    });
  };
  ...
  try {
    ...
    requests = allRequests.filter((req) => req.type === "تركيب");
  ...
  const installationCustomerSearchInput = document.getElementById(
    "installationCustomerSearchInput"
  );
  const installationCustomerDropdown = document.getElementById(
    "installationCustomerDropdown"
  );
  const installationCustomerIdInput = document.getElementById(
    "installationCustomerId"
  );
  function renderCustomerOptions(filtered) {
    installationCustomerDropdown.innerHTML = "";
    if (filtered.length === 0) {
      const noOption = document.createElement("div");
      noOption.className = "dropdown-item disabled";
      noOption.textContent = "لا يوجد نتائج";
      installationCustomerDropdown.appendChild(noOption);
      return;
    }
    filtered.forEach((customer) => {
      ...
      option.dataset.installationCustomerId = customer._id;
      option.addEventListener("click", () => {
        installationCustomerSearchInput.value = customer.name;
        installationCustomerIdInput.value = customer._id;
        installationCustomerDropdown.style.display = "none";
      });
      installationCustomerDropdown.appendChild(option);
    });
    installationCustomerDropdown.style.display = "block";
  }
  installationCustomerSearchInput.addEventListener("input", () => {
    const searchTerm = installationCustomerSearchInput.value
      .trim()
      .toLowerCase();
    if (searchTerm === "") {
      installationCustomerDropdown.style.display = "none";
  ...
  document.addEventListener("click", (e) => {
    if (
      !installationCustomerSearchInput.contains(e.target) &&
      !installationCustomerDropdown.contains(e.target)
    ) {
      installationCustomerDropdown.style.display = "none";
    }
  });
  ...
  const installationTechnicianSearchInput = document.getElementById(
    "installationTechnicianSearchInput"
  );
  const installationTechnicianDropdown = document.getElementById(
    "installationTechnicianDropdown"
  );
  const installationSelectedTechniciansContainer = document.getElementById(
    "installationSelectedTechnicians"
  );
  let installationSelectedTechnicians = [];
  function renderSelectedTechnicians() {
    installationSelectedTechniciansContainer.innerHTML = "";
    installationSelectedTechnicians.forEach((tech) => {
      ...
      badge.querySelector("button").addEventListener("click", () => {
        installationSelectedTechnicians =
          installationSelectedTechnicians.filter((t) => t._id !== tech._id);
        renderSelectedTechnicians();
      });
      installationSelectedTechniciansContainer.appendChild(badge);
    });
  }
  function renderTechnicianOptions(filtered) {
    installationTechnicianDropdown.innerHTML = "";
    if (filtered.length === 0) {
      const noOption = document.createElement("div");
      noOption.className = "dropdown-item disabled";
      noOption.textContent = "لا يوجد نتائج";
      installationTechnicianDropdown.appendChild(noOption);
      return;
    }
    filtered.forEach((technician) => {
      ...
      option.addEventListener("click", () => {
        if (
          !installationSelectedTechnicians.find((t) => t._id === technician._id)
        ) {
          installationSelectedTechnicians.push(technician);
          renderSelectedTechnicians();
        }
        installationTechnicianSearchInput.value = "";
        installationTechnicianDropdown.style.display = "none";
      });
      installationTechnicianDropdown.appendChild(option);
    });
    installationTechnicianDropdown.style.display = "block";
  }
  installationTechnicianSearchInput.addEventListener("input", () => {
    const searchTerm = installationTechnicianSearchInput.value
      .trim()
      .toLowerCase();
    if (searchTerm === "") {
      installationTechnicianDropdown.style.display = "none";
  ...
  document.addEventListener("click", (e) => {
    if (
      !installationTechnicianSearchInput.contains(e.target) &&
      !installationTechnicianDropdown.contains(e.target)
    ) {
      installationTechnicianDropdown.style.display = "none";
    }
  });
  ...
  function filterTable() {
    ...
    const installationPriority = document.getElementById(
    ...
    const filtered = requests.filter((req) => {
      ...
      return (
        ...
        (!installationPriority ||
          req.installationPriority === installationPriority) &&
        (!date || isSameDay(d1, d2)) &&
        (!search ||
          (req.installationCustomerId &&
            req.installationCustomerId.name.toLowerCase().includes(search)) ||
          (req.requestNumber &&
  ...
  document
    .getElementById("installationPriorityFilter")
    .addEventListener("change", filterTable);
  ...
  document
    .getElementById("addInstallationRequestForm")
    .addEventListener("submit", async (e) => {
      ...
      try {
        ...
        const newRequest = {
          ...
          type: "تركيب",
          installationCustomerId: document.getElementById(
            "installationCustomerId"
          ).value,
          installationPriority: document.getElementById("installationPriority")
            .value,
          installationDeviceType: document.getElementById(
            "installationDeviceType"
          ).value,
          installationDescription: document.getElementById(
            "installationDescription"
          ).value,
          status: "جديد",
          date: new Date().toISOString().split("T")[0],
          technicians:
            installationSelectedTechnicians.length > 0
              ? installationSelectedTechnicians.map((t) => t._id)
              : [],
          items:
        ...
        bootstrap.Modal.getInstance(
          document.getElementById("addInstallationRequestModal")
        ).hide();
        form.reset();
        form.classList.remove("was-validated");
        // Clear selected technicians and inventory
        if (typeof installationSelectedTechnicians !== "undefined") {
          installationSelectedTechnicians = [];
          document.getElementById("installationSelectedTechnicians").innerHTML =
            "";
        }
  ...
  window.viewRequest = async (id) => {
    try {
      ...
      // Set badge classes based on installationPriority and status
      const installationPriorityBadgeClass =
        {
          عالية: "bg-danger",
          متوسطة: "bg-warning",
          منخفضة: "bg-success",
        }[data.installationPriority] || "bg-secondary";
      ...
      document.getElementById("requestNumber").textContent =
        data.orderId || "N/A";
      document.getElementById("requestCustomer").textContent =
        data.installationCustomerId
          ? data.installationCustomerId.name
          : "غير متوفر";
      document.getElementById("requestDeviceType").textContent =
        data.installationDeviceType || "غير محدد";

      // Apply styled badges for installationPriority and status
      document.getElementById(
        "requestPriority"
      ).innerHTML = `<span class="badge ${installationPriorityBadgeClass}">${
        data.installationPriority || "غير محدد"
      }</span>`;
      document.getElementById(
        "requestStatus"
      ...
      if (!document.getElementById("requestType")) {
        ...
        const installationDeviceTypeDt = Array.from(
          dlRow.querySelectorAll("dt")
        ).find((dt) => dt.textContent.includes("نوع الجهاز"));
        if (installationDeviceTypeDt) {
          const installationDeviceTypeDd =
            installationDeviceTypeDt.nextElementSibling;
          dlRow.insertBefore(
            dtType,
            installationDeviceTypeDd.nextElementSibling
          );
          dlRow.insertBefore(
            ddType,
            installationDeviceTypeDd.nextElementSibling.nextElementSibling
          );
        } else {
          // If not found, append to the end
      ...
      document.getElementById("requestType").innerHTML = `<span class="badge ${
        data.type === "تركيب" ? "bg-info" : "bg-primary"
      ...
      // Format installationDescription with styling
      document.getElementById("requestDescription").innerHTML =
        data.installationDescription
          ? `<div class="p-2 bg-light border-start border-3 border-primary rounded">${data.installationDescription}</div>`
                  ...
                  <div class="row g-3">
                    ...
                    <div class="col-md-6">
                      ...
                      <select class="form-select" id="editPriority" required>
                        <option value="عالية" ${
                          data.installationPriority === "عالية"
                            ? "selected"
                            : ""
                        }>عالية</option>
                        <option value="متوسطة" ${
                          data.installationPriority === "متوسطة"
                            ? "selected"
                            : ""
                        }>متوسطة</option>
                        <option value="منخفضة" ${
                          data.installationPriority === "منخفضة"
                            ? "selected"
                            : ""
                        }>منخفضة</option>
                      </select>
      ...
      document
        ...
        .addEventListener("click", async () => {
          try {
            ...
            updatedOrder.installationCustomerId = data.installationCustomerId;
