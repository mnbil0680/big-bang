    ...
    return {
        ...
        async loadData() {
            try {
                ...
                // Load vouchers
                const vouchersResponse = await fetch('http://localhost:3000/vouchers');
        ...
        async deleteVoucher(voucher) {
            ...
            try {
                const response = await fetch(`http://localhost:3000/vouchers/${voucher._id}`, {
                    method: 'DELETE'
                });
        ...
        generateNewVoucherNumber() {
            const prefix = this.newVoucher.type === 'receipt' ? 
            ...
            const typeVouchers = this.vouchers.filter(v => v.type === this.newVoucher.type);
            const count = typeVouchers.length + 1;
            
            this.newVoucher.voucherNumber = `${prefix}${year}${month}-${count.toString().padStart(3, '0')}`;
        },
        ...
        createVoucherTypeChart() {
            const receiptTotal = this.receiptVouchers.reduce((sum, v) => sum + parseFloat(v.amount), 0);
            const paymentTotal = this.paymentVouchers.reduce((sum, v) => sum + parseFloat(v.amount), 0);
        ...
        createBalanceChart() {
            ...
            const sortedVouchers = [...this.vouchers].sort((a, b) => 
        ...
        get totalReceipts() {
            return this.receiptVouchers.reduce((sum, v) => sum + parseFloat(v.amount), 0);
        },
        
        // حساب إجمالي سندات الصرف
        get totalPayments() {
            return this.paymentVouchers.reduce((sum, v) => sum + parseFloat(v.amount), 0);
        },
        ...
        formatCurrency(amount) {
            ...
            return parseFloat(amount).toLocaleString('ar-SA') + ' ' + this.settings.currency;
        }
    };}
